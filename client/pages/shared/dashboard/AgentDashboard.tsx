...
import { APP_CONFIG, LOGIC_CONSTANTS, MOCK_DEFAULTS } from '@/constants';
...
export const AgentDashboard = ({ user }: AgentDashboardProps) => {
  ...
  const { data: intakeData, isLoading, execute: executeIntake } = useApi(api.fetchDailyIntake);
  ...
  if (showRecording) {
    return <GrainRecording onBack={() => setShowRecording(false)} onRefresh={executeIntake} />;
  }
